...
def calculate_global_production_impact():
    ...
    global_production_decline_absolute = (
        global_production_initial - global_production_after
    )
    global_decline_percent = (
        global_production_decline_absolute / global_production_initial
    ) * 100
    ...
    print(
        f"Absolute decline in US production: {us_production_decline_absolute:.2e} calories/year\n"
    )
    ...
    print(
        f"Expected global decline: {us_share_of_global:.1f}% × {us_decline_percent}% = {expected_global_decline:.2f}%"
    )
    print(f"Calculated global decline: {global_decline_percent:.2f}%")
    print(
        f"Match: {'✓' if abs(expected_global_decline - global_decline_percent) < 0.01 else '✗'}"
    )
