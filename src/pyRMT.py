...
def checkDesignMatrix(X):
    ...
    try:
        assert isinstance(
            X, (np.ndarray, pd.DataFrame, pd.Series, MutableSequence, Sequence)
        )
...
def clipped(X, alpha=None, return_covariance=False):
    ...
    if return_covariance:
        inverse_std = 1.0 / np.sqrt(np.diag(E))
        E *= inverse_std
    ...
    tmp = 1.0 / np.sqrt(np.diag(E_clipped))
    E_clipped *= tmp
...
def stieltjes(z, E):
    ...
    try:
        ...
        assert isinstance(E, (np.ndarray, pd.DataFrame, MutableSequence, Sequence))
...
def optimalShrinkage(X, return_covariance=False, method="rie"):
    ...
    if method is not "kernel":
        ...
        Gammas = map(
            lambda x: gammaHelper(
                x, q, N, lambda_N, inverse_wishart=use_inverse_wishart
            ),
            eigvals,
        )
...
def directKernel(q, T, N, eigvals):
    ...
    * "Direct Nonlinear Shrinkage Estimation of Large-Dimensional Covariance Matrices (September 2017)",
    ...
    tmp = np.sqrt(np.maximum(4 * square_Lt - (L - Lt) ** 2, zeros)) / (
        2 * np.pi * square_Lt
    )
    f_tilde = np.mean(tmp, axis=0)  # Equation (5.2)

    tmp = (
        np.sign(L - Lt) * np.sqrt(np.maximum((L - Lt) ** 2 - 4 * square_Lt, zeros))
        - L
        + Lt
    )
    ...
    if N <= T:
        ...
        Hf_tilde_0 = (
            (1 - np.sqrt(1 - 4 * h_squared))
            / (2 * np.pi * h_squared)
            * np.mean(1.0 / lambdas)
        )  # Equation (C.8)
        d_tilde_0 = 1 / (np.pi * (N - T) / T * Hf_tilde_0)  # Equation (C.5)
        d_tilde_1 = lambdas / (
            (np.pi**2) * (lambdas**2) * (f_tilde**2 + Hf_tilde**2)
        )  # Equation (C.4)
        d_tilde = np.concatenate(
            np.dot(d_tilde_0, np.ones(N - T, 1, np.float)), d_tilde_1
        )
