...
def main():
    ...
    avg_countries_per_decade = world_counts.groupby('decade')['Number of states in a region (Butcher and Griffiths)'].mean().round().astype(int)
    ...
    decade_data['Proportion'] = decade_data['Countries with shocks'] / decade_data['Total countries']
    ...
    for i, (bar, total, shocks) in enumerate(zip(bars, decade_data['Total countries'], decade_data['Countries with shocks'])):
    ...
    ax.set_title('Proportion of Countries Experiencing Their Largest Food Shock by Decade', fontsize=14, pad=20)
