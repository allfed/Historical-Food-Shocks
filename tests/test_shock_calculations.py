...
class TestCalculateFoodShocks:
    ...
    def actual_calorie_data(self):
        ...
        try:
            ...
            if data_path.exists():
                ...
                regions_to_remove = [
                    "Africa", "Americas", "Asia", "Caribbean", "Central America",
                    "Central Asia", "Eastern Africa", "Eastern Asia", "Eastern Europe",
                    "Europe", "Middle Africa", "Northern Africa", "Northern Europe",
                    "Oceania", "Polynesia", "South-eastern Asia", "Southern Africa",
                    "Southern Asia", "Southern Europe", "Western Africa", "Western Asia",
                    "Western Europe", "World", "South America",
                    "European Union (27)", "Land Locked Developing Countries",
                    "Least Developed Countries", "Low Income Food Deficit Countries",
                    "Net Food Importing Developing Countries", "Australia and New Zealand",
    ...
    def test_major_shock_detection(self, simple_test_data):
        ...
        result = calculate_changes_savgol(simple_test_data, window_length=15, polyorder=3)
        ...
        assert shock_magnitude < -15.0, f"Should detect major negative shock, got {shock_magnitude:.2f}%"
        
        # Most other years should have smaller deviations
        other_years = major_shock_data.drop(major_shock_data.index[shock_year_index])
        assert abs(other_years.mean()) < 3.0, "Non-shock years should have small deviations"
    ...
    def test_gcff_threshold_detection(self, simple_test_data):
        ...
        assert severe_negative_shocks >= 2, f"Should detect multiple negative GCFF-level shocks, found {severe_negative_shocks}"
        ...
        print(f"GCFF-level shocks (>5%): {severe_positive_shocks} positive, {severe_negative_shocks} negative")
        print(f"Major shocks (>10%): {major_positive_shocks} positive, {major_negative_shocks} negative")
        
        # Should have at least one major negative shock from our test data
        assert major_negative_shocks >= 1, "Should detect at least one major negative shock"
    ...
    def test_output_format_consistency(self, simple_test_data):
        ...
        assert result.shape == simple_test_data.shape, "Output shape should match input shape"
    ...
    def test_moderate_shock_detection(self, simple_test_data):
        ...
        assert min_shock_15 < -3.0, f"Should detect moderate shock around year 15, got {min_shock_15:.1f}%"
        assert min_shock_25 < -5.0, f"Should detect moderate shock around year 25, got {min_shock_25:.1f}%"
    ...
    def test_edge_cases(self):
        ...
        result_constant = calculate_changes_savgol(constant_data, window_length=3, polyorder=1)
        # All percentage changes should be zero (or very close to zero)
        assert abs(result_constant.values.max()) < 1e-10, "Constant data should yield zero changes"
    ...
    def test_realistic_country_patterns(self, actual_calorie_data):
        ...
        available_producers = [country for country in major_producers 
        ...
        for country in test_countries.index:
            ...
            assert shock_std > 0.5, f"{country} shows unusually low variability: {shock_std:.2f}%"
            assert shock_std < 50.0, f"{country} shows unusually high variability: {shock_std:.2f}%"
            ...
            print(f"{country}: {notable_shocks}/{total_years} years with >5% shocks ({shock_rate:.1%})")
            
            # Expect some but not too many notable shocks
            assert shock_rate > 0.02, f"{country} has too few notable shocks: {shock_rate:.1%}"
            assert shock_rate < 0.50, f"{country} has too many notable shocks: {shock_rate:.1%}"
